...
namespace messenger::handlers
{
  ...
  namespace
  {
    ...
    int ParseOffset(const HttpRequest &request)
    {
      ...
      const int v = std::stoi(arg);
      if (v < 0)
        throw ClientError(Msg("Invalid offset parameter"));
      return v;
    }
    ...
    public:
      ...
      std::string HandleRequestThrow(const HttpRequest &request, RequestContext &) const override
      {
        ...
        if (!user)
          throw ResourceNotFound(Msg("User not found"));
        request.GetHttpResponse().SetContentType("application/json");
        return JsonResponse(UserToJson(*user));
      }
    ...
    public:
      ...
      std::string HandleRequestThrow(const HttpRequest &request, RequestContext &) const override
      {
        ...
        if (!storage_.GetChat(chat_id))
          throw ResourceNotFound(Msg("Group chat not found"));
    ...
    public:
      ...
      std::string HandleRequestThrow(const HttpRequest &request, RequestContext &) const override
      {
        ...
        if (recipient == user_id)
          throw ClientError(Msg("Cannot message yourself"));
    ...
    public:
      ...
      std::string HandleRequestThrow(const HttpRequest &request, RequestContext &) const override
      {
        ...
        if (peer && *peer == user_id)
          throw ClientError(Msg("Invalid peer_id"));
        if (peer && !storage_.GetUser(*peer))
          throw ResourceNotFound(Msg("Peer not found"));
        const auto messages =
